...
# ─────────────────────────────────────────────
#  FLOATING MODE-SWITCHER CONTROL PANEL
# ─────────────────────────────────────────────
TOP_PICKS_URL = "https://www.linkedin.com/jobs/collections/top-applicant/"
_current_mode = [1]   # 1 = Top Picks Only  |  2 = Round Robin All Roles
_mode_root    = [None]
...
_board_stats = {
    "status":              "Starting...",
    "current_job":         "",
    "last_applied_list":   [],   # list of (title, company, datetime) — last 5
    "per_role":            {},   # role → {"checked": 0, "applied": 0}
    "easy_applied":        0,
    "external":            0,
    "failed":              0,
    "skipped":             0,
...
def _run_mode_window():
    root = tk.Tk()
    _mode_root[0] = root
    root.title("Job Applier Control")
    root.geometry("300x155+1100+10")
    root.attributes("-topmost", True)
    root.resizable(False, False)
    root.configure(bg="#1e1e2e")

    tk.Label(root, text="LinkedIn Auto Applier", font=("Segoe UI", 9),
             bg="#1e1e2e", fg="#888").pack(pady=(8, 0))

    mode_var  = tk.StringVar(value="Mode 1 — Top Picks Only")
    mode_lbl  = tk.Label(root, textvariable=mode_var,
                         font=("Segoe UI", 12, "bold"),
                         bg="#1e1e2e", fg="#89b4fa")
    mode_lbl.pack(pady=4)

    desc_var  = tk.StringVar(value="Applies from your LinkedIn recommended feed")
    tk.Label(root, textvariable=desc_var, font=("Segoe UI", 8),
             bg="#1e1e2e", fg="#a6adc8", wraplength=270).pack()

    btn_var = tk.StringVar(value="Switch → Round Robin All Roles")

    def toggle():
        if _current_mode[0] == 1:
            _current_mode[0] = 2
            mode_var.set("Mode 2 — Round Robin All Roles")
            mode_lbl.config(fg="#a6e3a1")
            desc_var.set("1 job per role: AI Eng → ML Eng → Backend → …")
            btn_var.set("Switch → Top Picks Only")
            btn.config(bg="#1565C0")
        else:
            _current_mode[0] = 1
            mode_var.set("Mode 1 — Top Picks Only")
            mode_lbl.config(fg="#89b4fa")
            desc_var.set("Applies from your LinkedIn recommended feed")
            btn_var.set("Switch → Round Robin All Roles")
            btn.config(bg="#2E7D32")

    btn = tk.Button(root, textvariable=btn_var, command=toggle,
                    font=("Segoe UI", 9, "bold"), fg="white", bg="#2E7D32",
                    padx=8, pady=6, relief="flat", cursor="hand2", bd=0,
                    activebackground="#1B5E20", activeforeground="white")
    btn.pack(pady=10, padx=20, fill="x")
    root.mainloop()

def start_mode_switcher():
    t = Thread(target=_run_mode_window, daemon=True)
    t.start()

def _run_stats_window():
    from datetime import datetime as _dt
    BG = "#1e1e2e"
    GREEN, RED, YELLOW, BLUE, TEXT, DIM = \
        "#a6e3a1", "#f38ba8", "#f9e2af", "#89b4fa", "#cdd6f4", "#6c7086"
    F = "DejaVu Sans"

    root = tk.Tk()
    root.title("Applier Stats")
    root.geometry("340x560+1560+10")
    root.attributes("-topmost", True)
    root.resizable(False, True)
    root.configure(bg=BG)

    def lbl(parent, text="", fg=TEXT, font=None, anchor="w", **kw):
        if font is None: font = (F, 9)
        return tk.Label(parent, text=text, fg=fg, bg=BG, font=font, anchor=anchor, **kw)

    def sep():
        tk.Frame(root, bg="#313244", height=1).pack(fill="x", padx=8, pady=3)

    # ── Header ──
    lbl(root, "LinkedIn Auto Applier", fg=DIM, font=(F, 8)).pack(pady=(8, 0))
    status_lbl = lbl(root, "● Starting...", fg=DIM, font=(F, 10, "bold"))
    status_lbl.pack()
    mode_lbl = lbl(root, "Mode: Top Picks", fg=BLUE, font=(F, 8))
    mode_lbl.pack(pady=(0, 4))

    sep()

    # ── Big stat counters ──
    sf = tk.Frame(root, bg=BG); sf.pack(fill="x", padx=10, pady=6)
    counter_lbls = {}
    for col, (key, caption, color) in enumerate([
        ("easy_applied", "✓ Applied",  GREEN),
        ("failed",       "✗ Failed",   RED),
        ("external",     "→ External", BLUE),
        ("skipped",      "⊘ Skipped",  YELLOW),
    ]):
        f = tk.Frame(sf, bg=BG); f.grid(row=0, column=col, padx=4, sticky="nsew")
        sf.columnconfigure(col, weight=1)
        lbl(f, caption, fg=DIM, font=(F, 7)).pack(anchor="w")
        n = tk.Label(f, text="0", fg=color, bg=BG, font=(F, 16, "bold"), anchor="w")
        n.pack(anchor="w")
        counter_lbls[key] = n
    ...
    # ── Last 5 applied ──
    lbl(root, "LAST APPLIED", fg=DIM, font=(F, 7, "bold")).pack(anchor="w", padx=10)
    last_rows = []
    for _ in range(5):
        r = tk.Label(root, text="—", fg=DIM, bg=BG, font=(F, 8), anchor="w",
    ...
    # ── Per-role table (Round Robin) ──
    lbl(root, "PER ROLE  (Round Robin)", fg=DIM, font=(F, 7, "bold")).pack(anchor="w", padx=10)
    role_frame = tk.Frame(root, bg=BG); role_frame.pack(fill="x", padx=10, pady=4)
    role_row_widgets = {}   # role → (title_lbl, seen_lbl, applied_lbl)
    _prev_role_keys = [None]

    def rebuild_role_table():
        for w in role_frame.winfo_children(): w.destroy()
        role_row_widgets.clear()
        roles = _board_stats["per_role"]
        if not roles:
            lbl(role_frame, "No role data yet", fg=DIM, font=(F, 8)).grid(row=0, column=0, sticky="w")
            return
        for col, (txt, width) in enumerate([("Role", 18), ("Seen", 5), ("Applied", 7)]):
            tk.Label(role_frame, text=txt, fg=DIM, bg=BG, font=(F, 7),
                     width=width, anchor="e" if col > 0 else "w").grid(row=0, column=col, sticky="ew", pady=(0, 2))
        role_frame.columnconfigure(0, weight=1)
        for i, (role, stats) in enumerate(roles.items(), 1):
            color = GREEN if stats["applied"] > 0 else TEXT
            short = role[:22] if len(role) > 22 else role
            tl = tk.Label(role_frame, text=short, fg=color, bg=BG, font=(F, 8), width=18, anchor="w")
            tl.grid(row=i, column=0, sticky="w")
            sl = tk.Label(role_frame, text=str(stats["checked"]), fg=DIM, bg=BG, font=(F, 8), width=5, anchor="e")
            sl.grid(row=i, column=1, sticky="e")
            al = tk.Label(role_frame, text=str(stats["applied"]), fg=color, bg=BG, font=(F, 8, "bold"), width=7, anchor="e")
            al.grid(row=i, column=2, sticky="e")
            role_row_widgets[role] = (tl, sl, al)

    sep()

    # ── Now checking ──
    lbl(root, "NOW CHECKING", fg=DIM, font=(F, 7, "bold")).pack(anchor="w", padx=10)
    now_lbl = tk.Label(root, text="—", fg="#a6adc8", bg=BG, font=(F, 8),
                       anchor="w", wraplength=320, justify="left")
    now_lbl.pack(anchor="w", padx=10, pady=(0, 8))

    def refresh():
        s = _board_stats
        # Status & mode
        m = get_current_mode()
        mode_lbl.config(text=f"Mode: {'Top Picks' if m == 1 else 'Round Robin'}")
        status_lbl.config(
            text=f"● {s['status']}",
            fg=GREEN if s["status"] not in ("Starting...", "Idle") else DIM
        )
        # Counters — direct config, no StringVar
        for key, widget in counter_lbls.items():
            widget.config(text=str(s[key]))
        # Last 5 applied
        entries = s["last_applied_list"]
        for i, row_lbl in enumerate(last_rows):
            if i < len(entries):
                title, company, t = entries[i]
                secs = int((_dt.now() - t).total_seconds())
                age = ("just now" if secs < 60 else
                       f"{secs // 60}m ago" if secs < 3600 else f"{secs // 3600}h ago")
                row_lbl.config(text=f"{title[:30]}  ·  {company[:18]}  ({age})", fg=TEXT)
            else:
                row_lbl.config(text="—", fg=DIM)
        # Now checking
        now_lbl.config(text=s["current_job"] or "—")
        # Per-role table
        cur_keys = list(s["per_role"].keys())
        if cur_keys != _prev_role_keys[0]:
            _prev_role_keys[0] = cur_keys[:]
            rebuild_role_table()
        else:
            for role, (tl, sl, al) in role_row_widgets.items():
                if role in s["per_role"]:
                    d = s["per_role"][role]
                    color = GREEN if d["applied"] > 0 else TEXT
                    tl.config(fg=color)
                    sl.config(text=str(d["checked"]))
                    al.config(text=str(d["applied"]), fg=color)
        root.after(1000, refresh)

    rebuild_role_table()
    root.after(200, refresh)   # let window fully render before first update
    root.mainloop()

def start_stats_board():
    t = Thread(target=_run_stats_window, daemon=True)
    t.start()
# ─────────────────────────────────────────────
...
# ── Enrich AI context with personal_details.md + projects.md ──
try:
    _ref_dir = "/home/har5ha/Desktop/resume/references"
    with open(f"{_ref_dir}/personal_details.md", encoding="utf-8") as _f:
        _personal_md = _f.read()
    with open(f"{_ref_dir}/projects.md", encoding="utf-8") as _f:
        _projects_md = _f.read()[:4000]   # first ~4k chars covers top projects
    user_information_all = (
        f"{_personal_md}\n\n---\n\n## Key Projects (excerpt)\n{_projects_md}"
    )
    print_lg("Loaded personal_details.md + projects.md for AI context.")
except Exception as _e:
    print_lg(f"Note: Could not load personal details MD files ({_e}), using questions.py fallback.")


def get_resume_for_role(search_term: str) -> str:
    """Return the resume path that best matches the current search term."""
    key = search_term.lower().strip()
    filename = resume_by_role.get(key, os.path.basename(default_resume_path))
...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    ...
    # Skip over-leveled titles before even clicking (Staff/Principal/VP/Director etc.)
    title_lower = title.lower()
    for bad_title in title_bad_words:
        if bad_title.lower() in title_lower:
            print_lg(f'Skipping "{title} | {company}" — over-leveled title ("{bad_title}"). Job ID: {job_id}!')
            skip = True
            break
    ...
    if not skip and company in blacklisted_companies:
        ...
        skip = True
    # Skip non-Easy Apply jobs directly from the card (before wasting time clicking in)
    if not skip and easy_apply_only:
        try:
            card_text = job.text
            if 'Easy Apply' not in card_text:
                print_lg(f'Skipping "{title} | {company}" — no Easy Apply badge on card. Job ID: {job_id}!')
                skip = True
        except: pass
    try:
...
def discard_job() -> None:
    try:
        ...
        if not wait_span_click(driver, 'Discard', 2):
            # LinkedIn may use a button with aria-label instead of a span
            try:
                driver.find_element(By.XPATH, '//button[@aria-label="Discard" or @data-test-dialog-primary-btn]').click()
            except: pass
        # Verify modal is gone; if still open, force-remove it via JS so the next job click isn't blocked
        try:
            modal = driver.find_element(By.XPATH, '//div[@data-test-modal-id="easy-apply-modal"]')
            driver.execute_script("arguments[0].remove();", modal)
            print_lg("Force-closed Easy Apply modal via JavaScript")
        except NoSuchElementException:
            pass  # Modal is gone — success
    except Exception as e:
        critical_error_log("In discard_job", e)
...
def apply_to_jobs(search_terms: list[str], max_per_term: int = None) -> None:
    applied_jobs = get_applied_job_ids()
    rejected_jobs = set()
    blacklisted_companies = set()
    global current_city, failed_count, skip_count, easy_applied_count, external_jobs_count, tabs_count, pause_before_submit, pause_at_failed_question, useNewResume, current_search_term, aiClient
    current_city = current_city.strip()
    _limit = max_per_term if max_per_term is not None else switch_number

    if randomize_search_order:  shuffle(search_terms)
    for searchTerm in search_terms:
        current_search_term = searchTerm
        useNewResume = True  # Re-upload resume for each search term (role-specific resume)
        # URL params: f_LF=f_AL (Easy Apply), f_TPR=r18000 (posted last 5hrs),
        # f_E=1,2,3,4 (Entry,Associate,Mid-Senior,Director), f_WT=1,2,3 (On-site,Remote,Hybrid)
        url_params = "&f_LF=f_AL&f_TPR=r18000&f_E=1,2,3&f_WT=1,2,3"
        if searchTerm.startswith("https://"):
            sep = "&" if "?" in searchTerm else "?"
            driver.get(searchTerm + sep + url_params.lstrip("&"))
            print_lg("\n________________________________________________________________________________________________________________________\n")
            print_lg(f'\n>>>> Now applying from URL: "{searchTerm}" <<<<\n\n')
        else:
            driver.get(f"https://www.linkedin.com/jobs/search/?keywords={searchTerm}{url_params}")
            print_lg("\n________________________________________________________________________________________________________________________\n")
        ...
        current_count = 0
        try:
            while current_count < _limit and (max_per_term is None or get_current_mode() == 1):
                ...
                for job in job_listings:
                    ...
                    if current_count >= _limit: break
                    # If user switched mode mid-pass, stop immediately so run() can apply the new mode
                    if max_per_term is not None and get_current_mode() != 1:
                        print_lg("Mode switched — stopping current pass to apply new mode.")
                        return
                    print_lg("\n-@-\n")
                    ...
                    if skip: continue
                    _board_stats["current_job"] = f"{title}  ·  {company}"
                    _board_stats["status"] = "Checking..."
                    ...
                    # Track per-role stats: job passed all filters, about to attempt apply
                    _role_key = current_search_term if not current_search_term.startswith("http") else "Top Picks"
                    if _role_key not in _board_stats["per_role"]:
                        _board_stats["per_role"][_role_key] = {"checked": 0, "applied": 0}
                    _board_stats["per_role"][_role_key]["checked"] += 1
                    _board_stats["status"] = f"Applying → {title[:30]}"

                    if use_AI and aiClient and description != "Unknown":
                        ##> ------ Yang Li : MARKYangL - Feature ------
                        try:
                            if ai_provider.lower() == "openai":
                            ...
                            # Detect API balance/auth errors in the returned dict and disable AI for the session
                            if isinstance(skills, dict) and "error" in skills:
                                err_str = str(skills["error"])
                                if any(code in err_str for code in ["402", "Insufficient Balance", "401", "invalid_api_key"]):
                                    print_lg(f"\n!!! AI API ERROR: {err_str}\n!!! Disabling AI for this session to avoid cascading failures. Top up your API balance and restart.\n")
                                    aiClient = None
                                    skills = "AI disabled — API balance error"
                                else:
                        ...
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    if useNewResume and not uploaded: uploaded, resume = upload_resume(modal, get_resume_for_role(current_search_term))
                    ...
                    if application_link == "Easy Applied":
                        ...
                        _board_stats["easy_applied"] += 1
                    else:
                        external_jobs_count += 1
                        _board_stats["external"] += 1
                    applied_jobs.add(job_id)
                    # Update stats board
                    _board_stats["last_applied_list"].insert(0, (title, company, datetime.now()))
                    _board_stats["last_applied_list"] = _board_stats["last_applied_list"][:5]
                    _board_stats["status"]            = f"Applied ✓  {title[:25]}"
                    if _role_key in _board_stats["per_role"]:
                        _board_stats["per_role"][_role_key]["applied"] += 1
...
def run(total_runs: int) -> int:
    ...
    for _cycle in range(25):
        ...
        if mode == 1:
            # Mode 1: Top Picks — process full list in one pass, then reload for next cycle
            print_lg(f"\n--- Mode 1: Top Picks | cycle {_cycle + 1}/25 ---")
            apply_to_jobs([TOP_PICKS_URL], max_per_term=99999)
        else:
            # Mode 2: Round Robin — 1 job per role, cycling across all search terms
            print_lg(f"\n--- Mode 2: Round Robin | cycle {_cycle + 1}/25 ---")
...
def main() -> None:
    ...
    try:
        ...
        missing = [p for p in set([os.path.join(RESUMES_DIR, f) for f in resume_by_role.values()] + [default_resume_path]) if not os.path.exists(p)]
        if missing:
            pyautogui.alert(text='Missing resume file(s):\n' + '\n'.join(missing) + '\n\nFor now the bot will use your previously uploaded LinkedIn resume.', title="Missing Resume(s)", button="OK")
            useNewResume = False
        ...
        # Launch floating control panel + stats board
        start_mode_switcher()
        start_stats_board()
        ...
        msg = f"Run complete. Check logs for details.\n\nSummary:\n{summary}"
